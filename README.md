...
| report.internalTemplate			| -reportInternalTemplate 				| Path (explicit or relative to src/main/resources) to template for internal only release notes. Freemarker is used as template engine. Variables provided in context: \$\{issues\} - map of issues where key is issue type and value list of issues; \$\{jiraUrl\} - url to jira; \$\{version\} - version for which release notes are generated | C:/releaseNotes/internalTemplate.ftl |
| report.externalTemplate			| -reportExternalTemplate 				| Path (explicit or relative to src/main/resources) to template for external release notes. Freemarker is used as template engine. Variables provided in context: \$\{issues\} - map of issues where key is issue type and value list of issues; \$\{jiraUrl\} - url to jira; \$\{version\} - version for which release notes are generated | C:/releaseNotes/externalTemplate.ftl |
